...
describe('concat', () => {
  ...
  it('concatenate two strings', () => {
    const result = concat("nitin ", "george");
    expect(result).to.equal("nitin george");
  });

  it('should throw an error for empty arguments', ()=> {
    expect(() => { concat("", "george") }).to.throw('strings should not be empty');
    expect(() => { concat("nitin ", "")}).to.throw('strings should not be empty');
  });
